"""Stage II trainer entry point: policy gradient with Pattern-Augmented Reward."""
...
def main():
    ...
    parser.add_argument("--config", default="configs/stage2_par.yaml")
    args = parser.parse_args()

    cfg = OmegaConf.load(args.config)

    os.environ["PATTERNBLOOM_STAGE"] = "2"
    ...
    os.environ["PATTERNBLOOM_GPM_PATH"] = cfg.gpm.memory_path
    os.environ["PATTERNBLOOM_GPM_ENCODER"] = cfg.gpm.encoder_name
    os.environ["PATTERNBLOOM_GPM_TOP_K"] = str(cfg.gpm.top_k)
    os.environ["PATTERNBLOOM_PAR_LAMBDA"] = str(cfg.par.lambda_p)
    ...
    verl_args = [
        ...
        f"trainer.experiment_name=stage2_par",
        f"reward_model.reward_manager=patternbloom_par",
